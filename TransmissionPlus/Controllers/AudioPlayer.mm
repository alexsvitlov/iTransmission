...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil file:(NSString*)url torrent:(Torrent *)t
{
    if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])) {
        ...
        for(NSUInteger i = 0; i < [[self.torrent flatFileList] count]; i++)
        {
            ...
            NSString *file = [[[(AppDelegate *)[UIApplication sharedApplication].delegate defaultDownloadDir] stringByAppendingPathComponent:[node path]] stringByAppendingPathComponent:[node name]];
