...
@property(nonatomic) UIImage* fIcon;
...
{
    self = [self initWithPath:nil hash:nil torrentStruct:nil magnetAddress:address lib:lib
        removeWhenFinishSeeding:nil
                 downloadFolder:location
         legacyIncompleteFolder:nil];
...
- (void)moveTorrentDataFileTo:(NSString*)folder
{
    ...
    {
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"A folder cannot be moved to inside itself.", "Move inside itself alert -> title") message:[NSString stringWithFormat:NSLocalizedString(@"The move operation of \"%@\" cannot be done.", "Move inside itself alert -> message"), self.name] preferredStyle:UIAlertControllerStyleAlert];
    ...
    {
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"There was an error moving the data file.", "Move error alert -> title") message:[NSString
                                                                                                                                                                                              stringWithFormat:NSLocalizedString(@"The move operation of \"%@\" cannot be done.", "Move error alert -> message"), self.name] preferredStyle:UIAlertControllerStyleAlert];
        [alertController addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"OK", "Move error alert -> button") style:UIAlertActionStyleDefault handler:nil]];
...
- (UIImage*)icon
{
    if (self.magnet)
    {
        return [UIImage imageNamed:@"Magnet"];
    }
    ...
    if (!self.fIcon)
    {
        ...
        NSURL *iconUrl = [NSURL fileURLWithPath:self.dataLocation];
        if (iconUrl) {
            self.fIcon = [FileUtils imageForPath:iconUrl.path];
        }
    }
    return self.fIcon;
}
...
- (NSInteger)checkForFiles:(NSIndexSet*)indexSet
{
    ...
    for (NSUInteger index = indexSet.firstIndex; index != NSNotFound; index = [indexSet indexGreaterThanIndex:index])
    {
        ...
        if (onState && offState)
        {
            return -1; //NSControlStateValueMixed //TODO: Alex
        }
    }
    //return onState ? NSControlStateValueOn : NSControlStateValueOff; //TODO: Alex
    return onState ? 1 : 0;
}
...
- (void)setFileCheckState:(NSInteger)state forIndexes:(NSIndexSet*)indexSet
{
    ...
    tr_torrentSetFileDLs(self.fHandle, files, count, state != 0 /*NSControlStateValueOff*/); //TODO: Alex
    free(files);
...
- (void)completenessChange:(tr_completeness)status wasRunning:(BOOL)wasRunning controller:(AppDelegate *)controller
{
...
//            [controller postFinishMessage:string];
